...
A lot of classical and deep learning models were invented and designed together with the data preprocessing and augmenting step \cite{bake_off, dl_tsc}. An example of such method is Window Slicing (\cite{dl_tsc}). This method extracts subsequences of of the original series. The resulting subsequences are then concatenenated with a downsampled copy and a copy that was smoothed. This method is used in Multi-scale Convolutional Neural Network (\cite{multiscale}) and Time Le-Net (\cite{lenet}).

Another preprocessing and augmentation method proposed together with the Time Le-Net model was Window Warping. It aims at making the model more robust to perturbations in the time axis, by training the network on \textit{squeezed} or \textit{dilated} series together with the original series. 
% TODO szczegoły
...
The first step of the algorithm is the data reduction step. Below we present the pseudocode in python.
\begin{lstlisting}[language=Python]
def get_prototypes(X, y):
	prototypes = []
	for class in set(y):
		X_class = X[y==class]
		medoid = get_medoid(X_class)
		prototype = DTW_averaging(data=X_class, init=medoid)
		prototypes.append(prototype)
	return prototypes

\end{lstlisting} 
We assume that the \texttt{get\_medoid} function, given a set of time series $\mathcal{X}$, returns a medoid with respect to the DTW distance, precisely:
$$medoid = \arg\min_{X \in \mathcal{X}} \sum_{\bar{X}\in \mathcal{X}} DTW(X, \bar{X})$$
The \texttt{DTW\_averaging} function performs a DTW barycenter averaging. The result of this function is a times series, that similarly to the medoid, minimizes the sum of DTW distances to each time series from the dataset. The difference between a DTW barycenter average and a medoid is that we do not restrict the result to be in from the original dataset, thus allowing to minimize the sum of distances more. The algorithm is described in detail in \cite{dtw_dba}.
This way we reduce the dataset to one prototype time series per each class. 

%\pagebreak
The next step is to compute the distance between datasets.
\begin{lstlisting}[language=Python]
def dist(X_1, y_1, X_2, y_2):
	distance = float("Inf")
	prototypes_1 = get_prototypes(X_1, y_1)
	prototypes_2 = get_prototypes(X_2, y_2)
	for prototype_1 in prototypes_1:
		for prototype_2 in prototypes_2:
			dtw_prototypes = DTW(prototype_1 prototype_2)
			dist = min(dist, dtw_prototypes)
	return dist
\end{lstlisting}
The distance is computed pairwise for all available datasets (using train splits). The authors compare the transfer learning performance of with randomly choosen source dataset versus choosing the source dataset that minimizes the former distance to the target dataset. The results are shown below (Figure \ref{fig:smart_transfer_learning}).
\FloatBarrier
...
The result show that choosing the source dataset for transfer learning based on similarity to the target dataset may lead to benefits. In this work we will try to construct the dataset artificially, by combining, preprocessing and augmenting the datasets from UCR archive, to mimic the properties of ImageNet dataset. Therefore we won't rather use this similarity to choose one particular dataset. We will take the inter-dataset distance into account when adjusting the distribution of classes in the artificial dataset, to favour classes similar to the target dataset.
% ------------------------------- BIBLIOGRAPHY ---------------------------
