...
For each comparative experiment we also performed a paired Wilcoxon test with the alternative hypothesis that one of the settings is stochatically greater than the other.
...
\FloatBarrier
We also performed a Wilcoxon test using validation accuracy of experiments conducted with \textit{baseline} transfer learning and without. The alternative hypothesis was that the validation accuracy of results conducted with transfer learning was greater than without transfer leaning. The p-value for results from $5th$ and $10th$ epoch were equal to \input{imgs/baseline/wilcoxon_greater_epoch_5.txt} and \input{imgs/baseline/wilcoxon_greater_epoch_10.txt}. Therefore we reject the null hypothesis, which states that the distributions are centered over the same value.
...
We also performed a Wilcoxon test using validation accuracy of experiments conducted with \textit{ensemble} transfer learning and without. The alternative hypothesis was that the validation accuracy of results conducted with transfer learning was greater than without transfer leaning. The p-value for results from $5th$ and $10th$ epoch were equal to \input{imgs/ensemble/wilcoxon_greater_epoch_5.txt} and \input{imgs/ensemble/wilcoxon_greater_epoch_10.txt}. Therefore we reject the null hypothesis for the $5th$ epoch, and assume that the distribution for the $10th$ epoch are centered over the same distribution.
...
\section{T-tests and Wilcoxon tests summary}
This section \dots %TODO: intro

\begin{table}[!h]
\caption{Summary of statistical tests.}
\centering
\tabcolsep=0.11cm
\scalebox {0.9}{\begin{tabular}{|l|l|l|ll|ll|l}
\cline{1-7}
\multirow{2}{*}{\textbf{Distribution 1}} &
  \multirow{2}{*}{\textbf{Distribution 2}} &
  \multirow{2}{*}{\textbf{Alternative}} &
  \multicolumn{2}{l|}{\textbf{p-value (epoch 5)}} &
  \multicolumn{2}{l|}{\textbf{p-value(epoch 10)}} &
  \textbf{} \\ \cline{4-7}
  &   &   & \multicolumn{1}{l|}{\textbf{wilcoxon}} & \textbf{t-test} & \multicolumn{1}{l|}{\textbf{wilcoxon}} & \textbf{t-test} &  \\ \cline{1-7}
\input{imgs/baseline/greater_test_table.txt}
\input{imgs/baseline/less_test_table.txt}
\input{imgs/baseline/two-sided_test_table.txt}
\input{imgs/baseline_vs_ensemble/greater_test_table.txt}
\input{imgs/baseline_vs_ensemble/less_test_table.txt}
\input{imgs/baseline_vs_ensemble/two-sided_test_table.txt}
\input{imgs/dba_vs_random/greater_test_table.txt}
\input{imgs/dba_vs_random/less_test_table.txt}
\input{imgs/dba_vs_random/two-sided_test_table.txt}
\input{imgs/ensemble/greater_test_table.txt}
\input{imgs/ensemble/less_test_table.txt}
\input{imgs/ensemble/two-sided_test_table.txt}
\input{imgs/ensemble_dba_3_vs_5/greater_test_table.txt}
\input{imgs/ensemble_dba_3_vs_5/less_test_table.txt}
\input{imgs/ensemble_dba_3_vs_5/two-sided_test_table.txt}
\input{imgs/ensemble_dba_5_vs_8/greater_test_table.txt}
\input{imgs/ensemble_dba_5_vs_8/less_test_table.txt}
\input{imgs/ensemble_dba_5_vs_8/two-sided_test_table.txt}
