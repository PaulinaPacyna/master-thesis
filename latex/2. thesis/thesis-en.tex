...
\begin{figure}[h!t]
\centering
\includegraphics[width=14 cm]{imgs/baseline/loss_acc.png}
...
\begin{figure}[h!t]
\centering
\includegraphics[width=15.5cm]{imgs/baseline/loss_acc.png}
...
\begin{figure}[h!t]
\centering
\includegraphics[width=14 cm]{imgs/ensemble/loss_acc.png}
...
For the baseline approach, the model is smaller, but the pre-training data is bigger than in the \textit{ensemble} approach. For the Fully Convolutional architecture, where the input size can be arbitrary, longer time series/more examples of time series impact the training time.  Therefore, the time needed to pre-train  the network in the \textit{baseline} approach is longer than in the \textit{ensemble} approach.

Training the model used in the \textit{ensemble} approach is composed of two steps. In the first step each component, which is a single FCN model, is trained on a single dataset, corresponding to each source dataset selected for the target dataset. Hence it is a single dataset and a single model, this step is relatively quick and could be parallelised, as the models and datasets and independent. The second step consist of training the model on the ensemble created from the pre-trained components on the target dataset. Although the model is bigger, we save time on the pre-training phase, comparing to the \textit{baseline} approach. Therefore in the end, the ensemble approach is faster. Figure \ref{fig:baseline_ensemble_traning_time} shows the comparison of training time for both approaches.  The training time shown on the plot is a sum of time needed for pre-training step and fine-tuning step.
...
\begin{figure}[h!t]
\centering
\includegraphics[width=17cm]{imgs/ensemble_vs_baseline/sim_acc.png}
\caption{}
...
The similarity measure is helpful, but it is a bit flawed. Due to the nature of the Dynamic Time Warping distance, the DBA similarity measure favours short time series. The following plot shows this: the mean of DBA distance from a given dataset to other dataset versus the mean series length in the dataset. This finding suggest using some kind of normalization (respect to series size) or trimming, when computing the similarity measure.
