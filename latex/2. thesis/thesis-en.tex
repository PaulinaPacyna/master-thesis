...
\noindent \textbf{Keywords:} time series, classification, transfer learning, deep learning, ensemble, \ldots % TODO ska supervisor why.. the template also has ", ..." at the end
\end{abstract}
}
...
\noindent \textbf{Słowa kluczowe:} szeregi czasowe, klasyfikacja, transfer learning, \ldots
\end{abstract}
}
...
\hline
\textbf{Dataset type (source)}       & \textbf{Number of datasets} \\ \hline
Audio                                & 10  \\ \hline
Device                               & 10                           \\ \hline
ECG (Electrocardiogram)              & 7 \\ \hline
EOG (Electrooculography)             & 2                           \\ \hline
EPG (Electrical penetration   graph) & 2                           \\ \hline
Hemodynamics                         & 3                           \\ \hline
Image                                & 33                          \\ \hline
Motion                               & 28 \\ \hline
Sensor                               & 22                          \\ \hline
Simulated                            & 9                           \\ \hline
...
Batch size                       & 32 \\ \hline
Augmentation rate                & 0.3 \\ \hline
Learning rate                    & 1e-5 with 0.75 decay over 10 000 steps (exponential decay) \\ \hline
Optimizer                        & Adam \\ \hline
Validation split rate            & 0.25 \\ \hline
...
The p-value of the Wilcoxon test with the alternative hypothesis that validation accuracy of experiments conducted using the DBA selection method than $0.05$, also indicating an advantage of the DBA selection method.
\FloatBarrier
\begin{figure}[h!t]
\centering
...
\hline
\multirow{2}{*}{\textbf{Distribution 1}} & \multirow{2}{*}{\textbf{Distribution 2}} & \multirow{2}{*}{\textbf{Alternative}} & \multicolumn{2}{l|}{\textbf{p-value}} \\ \cline{4-5}
  &   &   & \multicolumn{1}{l|}{\textbf{epoch 5}} & \textbf{epoch 10} \\ \hline

\input{imgs/baseline/greater_test_table.txt}
\input{imgs/baseline/two-sided_test_table.txt}
\input{imgs/baseline_vs_ensemble/greater_test_table.txt}
\input{imgs/baseline_vs_ensemble/two-sided_test_table.txt}
\input{imgs/dba_vs_random/greater_test_table.txt}
\input{imgs/dba_vs_random/two-sided_test_table.txt}
\input{imgs/ensemble/greater_test_table.txt}
\input{imgs/ensemble/two-sided_test_table.txt}
\input{imgs/ensemble_dba_3_vs_5/greater_test_table.txt}
\input{imgs/ensemble_dba_3_vs_5/two-sided_test_table.txt}
\input{imgs/ensemble_dba_5_vs_8/greater_test_table.txt}
5 datasets & 8 datasets & $\neq$ & \multicolumn{1}{l|}{\input{imgs/ensemble_dba_5_vs_8/wilcoxon_two-sided_epoch_5.txt}} & \input{imgs/ensemble_dba_5_vs_8/wilcoxon_two-sided_epoch_10.txt}\\\hline
\end{tabular}
}
\label{table:tests}
\end{table}
...
In this thesis, we extended the research on transfer learning in time series classification. We started by recalling the most popular methods used in time series classification and transfer learning. We used the UCR (University of California, Riverside \cite{UCR_archive}) time series archive and performed an explainatory data analysis on the available time series datasets. We described with properties of the source dataset are important for the transfer learning process. We outlined the phenomenon of negative transfer and strategies on mitigating the effect.
...
We also provided technical details on the code structure, preprocessing steps, model architecture, and hyperparameters. We conducted the analysis using the Fully Convolutional Network and used standard preprocessing steps like normalization or resampling. We utilized the property of Fully Convolutional Network which makes it possible to use different input lengths in every batch by implementing a custom Keras \texttt{DataGenerator}.

Furthermore, a detailed comparative analysis of the two approaches was conducted. Both \textit{baseline} and \textit{ensemble} approach proved to be equally efficient in terms of validation accuracy. However, the size of the ensemble model may be a limitation, especially with larger networks, bigger datasets, or environments with limited memory. Furthermore, the \textit{ensemble} approach is slightly slower, as we train multiple classifiers, and the resulting model is bigger. However, training the ensemble's component models can be parallelized, the components can be reused, and time can be saved.


We also considered two methods of selecting the datasets for pre-training the source classifiers. In one of the methods we selected the source datasets in order to minimize the DBA similarity measure to the target dataset. This method was compared to selecting the source dataset randomly from a given category. The method based on the DBA similarity measure has proven to be slightly advantageous compared to choosing the datasets randomly or by trial and error. However, the mean DBA distance between the source datasets and target dataset is not correlated with the accuracy of the target dataset. We believe that this method may be improved by enhancing the similarity measure.
...
%2. Proszę dodać pełne dane bibliograficzne do wszytskich pozycji z linkami DOI jeśli są dostępne.
