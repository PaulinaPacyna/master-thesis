...
\FloatBarrier
\section{Selecting the source datasets for multi-source transfer learning}
In both multi-source learning approached described above (\textit{baseline} and \textit{ensemble} approaches) we don't impose any restrictions on the source datasets itself. Neither of the two approaches don't include any rules how the source dataset correspond to the target dataset. In the experiments conducted in this thesis, two approaches for picking the datasets are proposed. In the first approach, we pick randomly $N$ datasets from a given category corresponding to the target dataset. To illustrate, let's say that the objective is to train a target classifier on the \texttt{InlineSkate} dataset, which belongs to the \texttt{Motion} category in the UCR archive. Then the source datasets are picked randomly from the \texttt{Motion}, which could be \texttt{UWaveGestureLibraryX, AsphaltPavementType, ToeSegmentation2, CricketZ, UWaveGestureLibraryZ}.

\chapter{Results}
The results and metrices of classifiers obtained from training networks with transfer learning were compared to the results obtained without transfer learning. It was important to always compare to a network that follows the same architecture as the target classifier. Therefore for e. g. the performance of the target classifier in the ensemble approach was compared to an ensemble model with the same amount of component models in the ensemble. Where applicable, the same hyperparameters were used for training with transfer learning and without transfer learning. This way the improvements can be attributed solely to the initial weights learned in the transfer learning procedure, not to ensembling, differences in model architecture or hyperparameters.
\section{Experiments}
