...
class Classifier_TLENET:
    ...
    def pre_processing(self, x_train, y_train, x_test, y_test):
        ...
        new_x_train = np.zeros(
            (x_train.shape[0] * tot_increase_num, length_ratio, x_train.shape[2])
        )
        new_y_train = np.zeros((y_train.shape[0] * tot_increase_num, y_train.shape[1]))

        new_x_test = np.zeros(
            (x_test.shape[0] * tot_increase_num, length_ratio, x_test.shape[2])
        )
        ...
        for i in range(x_train.shape[0]):
            for j in range(len(increase_nums)):
                ...
                new_x_train[idx : idx + increase_num, :, :] = x_train_augmented[j][
                    i * increase_num : (i + 1) * increase_num, :, :
                ]
                new_y_train[idx : idx + increase_num, :] = y_train_augmented[j][
                    i * increase_num : (i + 1) * increase_num, :
                ]
                idx += increase_num
        ...
        for i in range(x_test.shape[0]):
            for j in range(len(increase_nums)):
                increase_num = increase_nums[j]
                new_x_test[idx : idx + increase_num, :, :] = x_test_augmented[j][
                    i * increase_num : (i + 1) * increase_num, :, :
                ]
                new_y_test[idx : idx + increase_num, :] = y_test_augmented[j][
                    i * increase_num : (i + 1) * increase_num, :
                ]
                idx += increase_num
    ...
    def fit(self, x_train, y_train, x_test, y_test, y_true):
        ...
        for i in range(test_num_batch):
            unique_value, sub_ind, correspond_ind, count = np.unique(
                y_pred[i * tot_increase_num : (i + 1) * tot_increase_num],
                True,
                True,
                True,
            )
