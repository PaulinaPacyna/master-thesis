...
class Classifier_TWIESN:
    ...
    def train(self):
        ...
        df_val_metrics = calculate_metrics(
            np.argmax(self.y_val, axis=1), y_pred_val, 0.0
        )
        ...
        new_x_test = np.concatenate((self.x_test, state_matrix), axis=2).reshape(
            self.x_test.shape[0] * self.T, self.num_dim + self.N_x
        )
        # memory free
        state_matrix = None
        gc.collect()
