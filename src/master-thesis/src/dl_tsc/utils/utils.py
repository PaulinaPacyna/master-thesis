...
def read_dataset(root_dir, archive_name, dataset_name):
    ...
    if archive_name == "mts_archive":
        file_name = (
            cur_root_dir + "/archives/" + archive_name + "/" + dataset_name + "/"
        )
...
def read_all_datasets(root_dir, archive_name, split_val=False):
    ...
    if archive_name == "mts_archive":
        ...
        for dataset_name in DATASET_NAMES_2018:
            ...
            df_train = pd.read_csv(
                root_dir_dataset + "/" + dataset_name + "_TRAIN.tsv",
                sep="\t",
                header=None,
            )

            df_test = pd.read_csv(
                root_dir_dataset + "/" + dataset_name + "_TEST.tsv",
                sep="\t",
                header=None,
            )
...
def calculate_metrics(y_true, y_pred, duration, y_true_val=None, y_pred_val=None):
    res = pd.DataFrame(
        data=np.zeros((1, 4), dtype=np.float),
        index=[0],
...
def save_test_duration(file_name, test_duration):
    res = pd.DataFrame(
        data=np.zeros((1, 1), dtype=np.float), index=[0], columns=["test_duration"]
    )
...
def generate_results_csv(output_file_name, root_dir):
    res = pd.DataFrame(
        data=np.zeros((0, 7), dtype=np.float),
        index=[],
        columns=[
            "classifier_name",
            "archive_name",
    ...
    res = pd.DataFrame(
        {
            "accuracy": res.groupby(
                ["classifier_name", "archive_name", "dataset_name"]
            )["accuracy"].mean()
        }
    ).reset_index()
...
def visualize_filter(root_dir):
    ...
    for c in classes:
        ...
        plt.plot(
            c_x_train[idx], color=colors[idx_c], label="class" + str(idx_c) + "-raw"
        )
        plt.plot(
            convolved_filter_1[idx, :, idx_filter],
            color=colors_conv[idx_c],
