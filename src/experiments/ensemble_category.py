...
class EnsembleExperiment(BaseExperiment):
    def prepare_ensemble_model(self,
...
def train_ensemble_model(target_dataset: str, category: str):
    with mlflow.start_run(run_name="ensemble", nested=True):
        ...
        history = ensemble_model.fit(data_generator_train, epochs=2, validation_data=validation_data)
        mlflow_logging.log_confusion_matrix(
            *validation_data, classifier=ensemble_model, y_encoder=experiment.y_encoder
        )
        history = {
            f"ensemble_{key}": value for key, value in history.history.items()
        }
...
def train_plain_model(source_model: keras.models.Model, target_dataset: str) -> dict:
    with mlflow.start_run(run_name="plain model", nested=True):
        ...
        history = {
            f"ensemble_no_weights_{key}": value for key, value in history.history.items()
        }
        mlflow_logging.log_history(
            history,
        )
...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name=target_dataset):
        ensemble_training_results = train_ensemble_model(category=category, target_dataset=target_dataset)
        plain_training_results = train_plain_model(ensemble_training_results["model"], target_dataset=target_dataset)
