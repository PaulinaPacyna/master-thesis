...
client = MlflowClient()
runs = client.search_runs(["845246557029800560"])

history_detailed = []
for run in runs:
    if (
        run.info.status == "FINISHED"
        and run.info.run_name.startswith("Parent run")
        and run.info.lifecycle_stage == "active"
        and datetime.fromtimestamp(run.info.end_time / 1000)
        >= datetime(2023, 3, 14, 21, 28)
    ):
...
plt.ylim(bottom=0)
ax.legend()
plt.savefig("results/self_learning/loss.png")
plt.close(figure)
...
ax.set_xlabel("epoch")
ax.legend()
plt.savefig("results/self_learning/acc.png")
plt.close(figure)
...
def win_tie_loss_diagram(epoch):
    epoch_acc = [
        [
            stats["plain_val_accuracy"][epoch - 1],
            stats["self_learning_val_accuracy"][epoch - 1],
        ]
    ...
    ax.set_ylabel("With self learning")
    ax.set_xlabel("Without self learning")
    plt.xlim([0, 1])
    plt.ylim([0, 1])
    ax.set_aspect("equal")
    ax.text(
    ...
    plt.savefig(f"results/self_learning/win_tie_lose_epoch_{epoch}.png")
    plt.close(figure)
